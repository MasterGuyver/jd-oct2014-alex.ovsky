...
public class BattleFieldTemplate2 extends JPanel {
	...
	void runTheGame() throws Exception {
		...
		moveToQuadrant(1,1);
		int v = Integer.parseInt(getQuadrant(tankX,tankY).substring(0, separator));
		int u = Integer.parseInt(getQuadrant(tankX,tankY).substring(separator + 1));
		for(int i = u; i>=0; i--)
			if(battleField[v][i].equals("B")) battleField[v][i]="";
		for(int i = v; i>=0; i--)
			if(battleField[i][0].equals("B")) battleField[i][0]="";
		for(int i = 1; i<9; i++) {
			int flag = 0;
			for(int j=0; j<9; j++)
				if(battleField[j][i].equals("B")) flag++;
	...
	boolean processInterception() {
		...
		int v = Integer.parseInt(getQuadrant(bulletX,bulletY).substring(0, separator));
		int u = Integer.parseInt(getQuadrant(bulletX,bulletY).substring(separator + 1));
		if(v >= 0 && v<=9 && u >= 0 && u<=9) {
		if(battleField[v][u].equals("") || battleField[v][u].equals(" ")) return false;
		else battleField[v][u]=""; }
		else return false;
		return true;
	}
...
//----------------	
	...
	void fire() throws Exception {
		...
		bulletY = tankY+25;
		int step = 1;
		while ((bulletX >= 0 && bulletX < 562) && (bulletY >= 0 && bulletY < 562)) {
			if(tankDirection == 1) bulletY-= step;
			else if(tankDirection == 2) bulletY+= step;
			else if(tankDirection == 3) bulletX-= step;
			else bulletX+=step;
